...
from exposure import calculate_exposure, best_activity_window, 
ACTIVITY_MET
...
def get_live_aqi():
    url = 
f"https://api.openweathermap.org/data/2.5/air_pollution?lat={DELHI_LAT}&lon={DELHI_LON}&appid={OPENWEATHER_KEY}"
    return requests.get(url).json()

def get_forecast():
    url = 
f"https://api.openweathermap.org/data/2.5/air_pollution/forecast?lat={DELHI_LAT}&lon={DELHI_LON}&appid={OPENWEATHER_KEY}"
    return requests.get(url).json()
